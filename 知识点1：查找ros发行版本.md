...
![image-20240604162207705](知识点1：查找ros发行版本.assets/image-20240604162207705.png)

分别对应于FMU通道的1234567.为速度控制。
...
> ## Excerpt
> Linux  系统目录结构 登录系统后，在当前命令窗口下输入命令：  ls / 你会看到如下图所示:  树状目录结构：  以下是对这些目录的解释： /bin： bin 是 Binaries (二进制文件) 的缩写, 这个目录存放着最经常使用的命令。 /boot：这里存放的是启动 Linux 时使用的一些核心文件，包括一些连接文件以及镜像文件。 /dev ：dev 是 Device(设备) 的缩写, 该目录下存放的是 Linux 的外部设..

---
登录系统后，在当前命令窗口下输入命令：

```
<span>&nbsp;ls </span><span>/</span><span>&nbsp;</span>
```

你会看到如下图所示:

![](知识点1：查找ros发行版本.assets/4_20.png)

树状目录结构：

![](知识点1：查找ros发行版本.assets/d0c50-linux2bfile2bsystem2bhierarchy.jpg)
...
-   **/proc**：  
    ...
    ```
    <span>echo </span><span>1</span><span> </span><span>&gt;</span><span> </span><span>/proc/</span><span>sys</span><span>/</span><span>net</span><span>/</span><span>ipv4</span><span>/</span><span>icmp_echo_ignore_all</span>
    ```
...
有时CTRL+c杀不掉进程；此时可用CTRL+Z先暂停到后台，然后kill  -9 %

或者

先使用以下代码来显示目前进程，然后读取ID，再kill
